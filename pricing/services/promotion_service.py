...
class PromotionService:
    ...
    def get_promotional_price(
        ...
        """
        LEGACY METHOD: Use get_best_promotional_price() for new code

        Maintained for backward compatibility
        """
        result = PromotionService.get_best_promotional_price(location, product, quantity, date, customer)
        if result.ok and result.data.get('has_promotion'):
            return result.data.get('promotional_price')
        else:
            return None
    ...
    def get_active_promotions(location=None, product=None, date=None) -> List[Dict]:
        """
        LEGACY METHOD: Use get_active_promotions_summary() for new code

        Maintained for backward compatibility
        """
        result = PromotionService.get_active_promotions_summary(location, product, date)
        if result.ok:
            return result.data.get('active_promotions', [])
        else:
            logger.error(f"Error getting active promotions: {result.msg}")
            return []

    @staticmethod
    def validate_promotion_data(promo_data: Dict) -> tuple[bool, List[str]]:
        """
        LEGACY METHOD: Use validate_promotion_setup() for new code

        Maintained for backward compatibility
        """
        result = PromotionService.validate_promotion_setup(promo_data)
        if result.ok:
            return True, result.data.get('validation_warnings', [])
        else:
            return False, result.data.get('validation_errors', [result.msg])

    @staticmethod
    def deactivate_expired_promotions() -> int:
        """
        LEGACY METHOD: Use manage_promotion_lifecycle() for new code

        Maintained for backward compatibility
        """
        result = PromotionService.manage_promotion_lifecycle()
        if result.ok:
            return result.data.get('expired_deactivated', 0)
        else:
            logger.error(f"Error deactivating expired promotions: {result.msg}")
            return 0
